...
import { DutchAuctionERC20 } from "../typechain-types/contracts/ERC20Token.sol/DutchAuctionERC20";
...
describe("DutchAuction", function () {
  ...
  {
      it("Check if the NFT is successfully transferred in the auction contract", async function () 
      {
        ...
        await auctionContract.connect(firstAcc).OwnershipTransfer();
      ...
      it("Check if the auction initiater holds the auction NFT", async function () 
      {
        ...
        await expect(auctionContract.OwnershipTransfer()).revertedWith("Only owner of the NFT can start the auction.");
      });
      ...
      it("Check if the NFT is transferred in the auction smart contract", async function () 
      {
        const { auctionContract, owner, firstAcc } = await loadFixture(deployDutchAuctionTestFixture);
  
        await expect(auctionContract.connect(owner).endAuction()).revertedWith("Auction NFT is not transferred.");
      });
  });
